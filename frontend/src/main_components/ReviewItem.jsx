...
const ReviewItem = ({ review }) => {
  ...
  useEffect(() => {
    // 초기 좋아요 수 로딩
    axios
      .get(`http://localhost:8082/controller/${review.logIdx}/likes`)
      .then((res) => setLikes(res.data))
      .catch((err) => console.error(err));

    // 댓글 수 로딩 - 피드 데이터에서 댓글 정보 가져오기
    axios
      .get(`http://localhost:8082/controller/feed/${review.logIdx}`)
      .then((res) => {
        if (res.data && res.data.length > 0) {
          const logData = res.data[0];
          // 댓글 배열이 있으면 그 길이를 사용, 없으면 0으로 설정
          setCommentCount(
            Array.isArray(logData.comments) ? logData.comments.length : 0
          );
        }
      })
    ...
    if (user) {
      axios
        .get(
          `http://localhost:8082/controller/isLiked?logIdx=${review.logIdx}&userId=${user.userId}`
        )
    ...
    <div className="review-wrapper">
      ...
      <div className="review-header">
        ...
        >
          ...
          <div className="username" style={{ marginLeft: "0px" }}>
            {userId}
          </div>
        </div>
      ...
      >
        ...
        >
          ...
          <div style={{ fontSize: "0.95rem", color: "#555" }}>{author}</div>
